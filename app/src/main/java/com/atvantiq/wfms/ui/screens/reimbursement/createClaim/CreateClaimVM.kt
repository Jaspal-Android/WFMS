...
class CreateClaimVM @Inject constructor(
    ...
    fun onSubmitClaim() {
        ...
        val otherJson = JSONArray().apply {
            othersEntriesList.value.orEmpty().forEachIndexed { i, o ->
                ...
                o.receiptAttachments.orEmpty().forEachIndexed { j, path ->
                    val key = "${nextKey("other${i + 1}_", j + 1)}" // file_other1_1 (adjust if backend requires file_other_1 style)
                    attachmentKeys += key
                    addFilePart(key, File(path))
                }
        ...
        val dataJson = JSONObject()
            ...
            .put("expense_category", isOutstationExpense.get()?.let { if (it) "outstation" else "local" } ?: "local")
